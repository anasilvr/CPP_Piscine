...
int main () {
	...
	try {
		...
		std::cout << "\t[" << "4294967295" << "] " << test2[-2] << std::endl;
	}
	catch (std::exception &e) {
		std::cerr << e.what() << std::endl;
	}
	...
	try {
		...
		std::cout << "test4 content and addresses:\t\ttest4copy content and addresses\t\tanothercopy content and addresses" << std::endl;
		for(unsigned int i = 0; i < test4.size(); i++) {
			std::cout << "[" << i << "] " << test4[i] << &test4[i];
			std::cout << "\t\t\t[" << i << "] " << test4copy[i] << &test4copy[i];
			std::cout << "\t\t\t[" << i << "] " << anothercopy[i] << &anothercopy[i] << std::endl;
